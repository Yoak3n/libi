...
func GetUserDetail(uid int64) ([]byte, error) {
	urlStr := fmt.Sprintf("https://api.bilibili.com/x/space/wbi/acc/info?mid=%d", uid)
	return GetWithWbi(urlStr)
}
...
func updateCache() {
	last := time.Unix(config.Conf.Auth.LastUpdate, 0)
	if time.Since(last).Hours() < 12 {
		cache.Store("imgKey", config.Conf.Auth.ImgKey)
		cache.Store("subKey", config.Conf.Auth.SubKey)
		return
	}
	...
	mux.Lock()
	config.SetWBIKey(imgKey, subKey)
	mux.Unlock()
}

func getWbiKeysCached() (string, string) {
	updateCache()
	img, _ := cache.Load("imgKey")
	sub, _ := cache.Load("subKey")
	return img.(string), sub.(string)
}
